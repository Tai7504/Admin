              ...
              leads.map((lead) => (
                <tr key={lead.id} className="hover:bg-gray-50 transition-colors">
                  ...
                  <td className="px-6 py-4 font-medium text-blue-600">
                    <div>{lead.course_interested?.name || 'Chưa chọn'}</div>
                    {lead.applied_voucher_code && (
                      <div className="text-xs mt-1.5 space-y-0.5 font-normal">
                        <span className="inline-block bg-red-50 text-red-600 border border-red-200 px-1.5 py-0.5 rounded font-mono font-bold text-[10px]">
                          🎟️ {lead.applied_voucher_code}
                        </span>
                        <div className="text-gray-500">
                          <span className="line-through">{Number(lead.original_price).toLocaleString('vi-VN')}đ</span>
                          <span className="text-red-600 font-bold ml-1">{Number(lead.final_price).toLocaleString('vi-VN')}đ</span>
                        </div>
                      </div>
                    )}
                  </td>
            ...
            <div className="p-8 space-y-8">
              ...
              <div>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  <div className="bg-gradient-to-r from-green-50 to-green-100 p-6 rounded-xl border border-green-200 flex flex-col justify-center">
                    <label className="text-xs font-semibold text-green-700 uppercase tracking-wider block mb-1">Khóa học đăng ký</label>
                    <p className="text-lg font-bold text-green-950">
                      {selectedLead.course_interested?.name || 'Chưa chọn khóa học'}
                    </p>
                  </div>

                  {selectedLead.applied_voucher_code ? (
                    <div className="bg-red-50 p-6 rounded-xl border border-red-200">
                      <label className="text-xs font-semibold text-red-700 uppercase tracking-wider block mb-1">Mã Voucher áp dụng</label>
                      <p className="text-lg font-bold text-red-700 font-mono">
                        🎟️ {selectedLead.applied_voucher_code}
                      </p>
                      <div className="mt-2 text-xs">
                        <span className="text-gray-500">Học phí gốc: </span>
                        <span className="line-through font-semibold text-gray-700">{Number(selectedLead.original_price).toLocaleString('vi-VN')} đ</span>
                      </div>
                      <div className="mt-0.5 text-sm">
                        <span className="text-gray-900 font-bold">Thực thu sau giảm: </span>
                        <span className="text-red-600 font-extrabold text-base">{Number(selectedLead.final_price).toLocaleString('vi-VN')} đ</span>
                      </div>
                    </div>
                  ) : (
                    <div className="bg-gray-50 p-6 rounded-xl border border-gray-200 flex flex-col justify-center">
                      <label className="text-xs font-semibold text-gray-500 uppercase tracking-wider block mb-1">Học phí gốc</label>
                      <p className="text-lg font-bold text-gray-900">
                        {selectedLead.original_price ? `${Number(selectedLead.original_price).toLocaleString('vi-VN')} đ` : (selectedLead.course_interested?.price ? `${Number(selectedLead.course_interested.price).toLocaleString('vi-VN')} đ` : 'Chưa cập nhật')}
                      </p>
                      <span className="text-xs text-gray-500 mt-0.5">(Không áp dụng voucher)</span>
                    </div>
                  )}
                </div>
              </div>
